...
def validate_roles_data(raw: Any, *, strict: bool = True) -> None:
    ...
    for i, item in enumerate(raw):
        if not isinstance(item, dict):
            errors.append(f"[{i}] Each role must be an object.")
            continue
        ...
        iterations = item.get("iterations", 200_000)
        ...
        if not isinstance(name, str) or not name.strip():
            ...
            if key in seen_names:
                errors.append(f"[{i}] Duplicate role name (case-insensitive): {name!r}.")
            seen_names.add(key)
        ...
        if not isinstance(rid, int):
            ...
            if rid in seen_ids:
                errors.append(f"[{i}] Duplicate role id: {rid}.")
            seen_ids.add(rid)
...
def validate_permissions_data(raw: Any) -> None:
    ...
    if perms is None:
        ...
        for key, rec in perms.items():
            if not isinstance(key, str) or not key.strip():
                errors.append("Permission keys must be non-empty strings.")
                continue
            if not isinstance(rec, dict):
                errors.append(f"Permission {key!r} must be an object.")
                continue
    ...
    if errors:
        raise PermissionsValidationError(errors)
